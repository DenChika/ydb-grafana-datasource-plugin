...
| `$__dateFilter(columnName)`                 | Replaced by a conditional that filters the data (using the provided column) based on the date range of the panel                               | `foo >= CAST(2021-11-12 AS Date) AND foo <= CAST(2022-11-12 AS Date)`                                                                             |
| `$__dateTimeFilter(dateColumn, timeColumn)` | Shorthand that combines $__dateFilter() AND $__timeFilter() using separate Date and DateTime columns                                           | `dc >= CAST(2021-11-12 AS Date) AND dc <= CAST(2022-11-12 AS Date) AND tc >= CAST(1636717526 AS Datetime) AND tc <= CAST(1668253526 AS Datetime)` |
